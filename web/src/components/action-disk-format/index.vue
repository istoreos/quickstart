<template>
    <action-component :type="1">
        <transition name="rotate" mode="out-in">
            <form class="action format" @submit.prevent="onSumbit" v-if="setup == 0">
                <div class="action-header">
                    <div class="action-header_title">{{ $gettext("硬盘配置") }}</div>
                </div>
                <div class="action-body">
                    <div class="disk-info">
                        <div class="disk-info_icon">
                            <svg t="1642589762094" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                xmlns="http://www.w3.org/2000/svg" p-id="11301" width="128" height="128">
                                <path
                                ...
                                <path
                                    ...
                                    fill="#6E6E96" p-id="11305" />
                                <path d="M301.946104 941.515457h429.985632v65.841173H301.946104z" fill="#8A8AA1"
                                    p-id="11306" />
                                <path
                                ...
                                <path
                                    ...
                                    fill="#6E6E96" p-id="11319" />
                                <path d="M301.946104 941.515457h429.985632v36.977408H301.946104z" fill="#6E6E96"
                                    p-id="11320" />
                            </svg>
                        </div>
                        <div class="disk-info_mount-name" v-if="mount">
                            <span>【{{ mount.total }}】</span>
                            <span>{{ mount.mountPoint || mount.path }}</span>
                        </div>
                        <div class="disk-info_mount-name" v-else-if="disk">
                            <span>【{{ disk.size }}】</span>
                            <span>{{ disk.venderModel }}</span>
                        </div>
                    </div>

                    <div class="label-item" v-if="mount">
                        <div class="label-item_key">
                            <span>{{ $gettext("目标分区") }}</span>
                        </div>
                        <div class="label-item_path">{{ mount.mountPoint || mount.path }}（{{ mount.total }}）</div>
                    </div>

                    <div class="label-item">
                        <div class="label-item_key">
                            <span>{{ $gettext("格式化选项") }}</span>
                        </div>
                        <div class="label-item_value">
                            <select v-model="value" required :disabled="action == 'disk'">
                                <option value v-if="mount != null">{{ $gettext("请选择选项") }}</option>
                                <option value="format">{{ $gettext("格式化") }}</option>
                                <option value="default" v-if="mount != null">{{ $gettext("不格式化,使用原文件系统") }}</option>
                            </select>
                        </div>
                        <div class="label-item_value">
                            <p class="msg" v-if="value == 'format'">{{ $gettext("格式化为EXT4文件系统") }}</p>
                            <p class="msg" v-else-if="value == 'default'"></p>
                        </div>
                    </div>
                </div>
                <div class="action-footer">
                    <div class="auto"></div>
                    <button class="cbi-button cbi-button-remove app-btn app-back" @click="onCancel" type="button"
                        :disabled="disabled">{{ $gettext("返回") }}</button>
                    <button class="cbi-button cbi-button-apply app-btn app-next" :disabled="disabled">{{ $gettext("下一步") }}</button>
                </div>
            </form>
            <div class="action result" v-else-if="setup == 1">
                <div class="action-body">
                    <div class="action-body_icon">
                    ...
                    <div class="action-body_msg">{{ $gettext("格式化成功") }}</div>
                    <div class="action-body_info" v-if="resultDisk">
                        {{ $gettext("已经成功格式化磁盘") }} {{ resultDisk.venderModel }} {{ $gettext("并挂载到") }}
                        <a>{{ resultMount?.mountPoint }}</a>
                    </div>
                    <div class="action-body_info" v-if="resultMount">
                        {{ $gettext("已经成功初始化分区") }}
                        <a>{{ resultMount.mountPoint }}</a>
                    </div>
                    <div class="btns">
                        <button class="cbi-button cbi-button-apply app-btn app-next" type="button" @click="onSuccess">{{
                                action == 'nas' ? $gettext('下一步') : $gettext('完成')
                        }}</button>
                    </div>
                </div>
            </div>
...
const onNext = (_rootPath: string) => {
    if (props.Next) {
        props.Next(_rootPath)
    }
    onClose()
    return
}
...
const setup = ref(0)
const onSetup = (_setup: number) => {
    setup.value = _setup
}
const value = ref(props.action == "nas" ? "" : "format")
const resultDisk = ref<NasDiskModel | null>()
const resultMount = ref<MountPoint | null>()
const onSumbit = () => {
    switch (value.value) {
        case "format":
            onFormat()
...
const onDefault = () => {
    let rootPath = ""
    const _mount = props.mount
    if (_mount != null && _mount.mountPoint != null) {
        rootPath = _mount.mountPoint
    }
    if (rootPath != "") {
        onNext(rootPath)
        return
    }
    Toast.Warning($gettext("无法识别路径"))
}
// 选择格式化硬盘
const onFormat = () => {
    const _disk = props.disk
    const _mount = props.mount
    // 格式化分区
    if (_mount) {
        const partname = _mount.mountPoint || _mount.path
        if (!confirm($gettext("警告：格式化会清空 %{partname} 分区数据，请你谨慎操作", {partname:partname||''}))) {
            
            return
        }
        if (!confirm($gettext("是否确定格式化 %{partname}?", {partname:partname||''}))) {
            return
        }
        onFormatMount(_mount)
        return
    }
    // 初始化
    if (_disk) {
        if (!confirm($gettext("警告：该操作将初始化 %{model} 硬盘并创建分区，请你谨慎操作", {model:_disk.venderModel||''}))) {
            return
        }
        if (!confirm($gettext("是否确定初始化?"))) {
            return
        }
        onFormatDisk(_disk)
        return
    }
    Toast.Warning($gettext("无法识别数据"))
    return
}
// 初始化硬盘
const onFormatDisk = async (_disk: NasDiskModel) => {
    if (_disk.name == null || _disk.path == "") {
        Toast.Warning($gettext("获取不到设备名称"))
        return
    }
    if (_disk.path == null || _disk.path == "") {
        Toast.Warning($gettext("获取不到设备路径"))
        return
    }
    disabled.value = true
    const load = Toast.Loading($gettext("初始化中..."))
    try {
        const res = await request.Nas.Disk.Init.POST({
            name: _disk.name,
            path: _disk.path,
        })
        if (res?.data) {
            ...
            if (result) {
                if (result.errorInfo) {
                    Toast.Warning(result.errorInfo)
                } else {
                    // 初始化成功
                    Toast.Success($gettext("初始化成功"))
                    if (result.childrens && result.childrens.length > 0) {
                        resultMount.value = result.childrens[0]
                    }
                    resultDisk.value = result
                    onSetup(1)
                }
            }
        }
...
// 格式化当前分区
const onFormatMount = async (_mount: MountPoint) => {
    if (_mount.path == null || _mount.path == "") {
        Toast.Warning($gettext("获取不到分区路径"))
        return
    }
    disabled.value = true
    const load = Toast.Loading($gettext("格式化中..."))
    try {
        const res = await request.Nas.Disk.Partition.Format.POST({
            path: _mount.path,
            uuid: _mount.uuid,
        ...
        if (res?.data) {
            ...
            if (result) {
                Toast.Success($gettext("格式化成功"))
                resultMount.value = result
                onSetup(1)
            }
        }
    } catch (error) {
...
const onSuccess = () => {
    if (resultMount.value) {
        if (resultMount.value.mountPoint) {
            onNext(resultMount.value.mountPoint)
            return
        }
    }
    Toast.Warning($gettext("读取结果失败"))
}
</script>
